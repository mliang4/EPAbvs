...
bvsEPA = function(
  ...
  n_i = NULL,
  n_betn = NULL,
  flex_covas = NULL, # flexible covariates (trivial cluster allowed)
  ...
  if( is.null(n_betn) ){ n_betn = round(N/10) }
  if( is.null(flex_covas) ){ flex_covas = c(1:(P-1)) }
  if( is.null(between_subj) ){ between_subj = c(0:(N-1)) }
