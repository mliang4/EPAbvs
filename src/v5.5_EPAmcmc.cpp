...
double log_prob_clust_single(
  ...
  if ( sigma_i > 0 ){ 
    ...
    if ( j.size() > 0 ){ // If there is a match in the cluster membership
      p_return = ( sigma_i - d_p*q ) / ( a_p + sigma_i ) * sum( lambda_dist_pi( j ) ) / sum( lambda_dist_pi( sigma_vec ) );
    }else{
...
List between_step(
  ...
  if( gamma_temp( i, p ) == 0 ){
    ...
    beta_proposal( i, span( 3*p, 3*p+2 ) ) = beta_proposal_ip;
    
    double r =
      help::log_like_logit_single(i, subject, Y, Xbar, Ustar, Ustar_dems, beta_proposal, xi_temp)
      + help::log_mvnorm_cpp(beta_proposal_ip.t(), zero_vec3, var_diag)
      + help::log_beta_binomial_cpp(1, a_gamma, b_gamma)
      - help::log_like_logit_single(i, subject, Y, Xbar, Ustar, Ustar_dems, beta_temp, xi_temp)
      ...
      if( a < r ){
        ...
        s_temp( i, p ) = max(sp) + 1; // assign this subject to a new cluster
        gamma_temp( i, p ) = 1;
      }
  ...
  for( int iter = 0; iter < iterations; ++iter ){
    ...
    for( int p = 0; p < P; ++p ){
      ...
      for ( int i = 0; i < N; ++i ){
        ...
        return_update_spi = help::update_spi_EPA(i, p, between_covas, subject, subject_dems, Y, Xbar, Ustar, Ustar_dems, beta_temp, xi_temp, s_temp, gamma_temp, w, alpha, delta, sigma, mu, lambda2_temp, tau2_temp, lambda_dist_p);
